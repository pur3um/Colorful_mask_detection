...
def temp(request):
    ...
    # print(data)
    data = data[22:]  # 앞의 'data:_image/png;base64' 부분 제거
    number = random.randrange(1,10000) # 동시에 다른 사용자가 접근시 최대한 중복을 막기

    # 저장할 경로 및 파일명을 지정
    path = str(os.path.join(settings.STATIC_ROOT, '_image'))
    filename = '_image' + str(number) + '.png'
...
#     # 바이너리 쓰기'wb'모드로 파일 open
#     _image = open(path + filename, 'wb')
#     # 'base64.b54decode()'메소드로 파일 디코딩 후 파일쓰기
#     _image.write(base64.b64decode(data))
#     _image.close()
